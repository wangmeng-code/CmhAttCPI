...
class CmhAttCPI(nn.Module):
    def __init__(self, init_atom_features, init_bond_features, num_features_xd, embed_dim, dropout, \
        ...
        self.lstm = nn.LSTM(input_size = self.embed_dim, hidden_size=self.hidden_size, num_layers=self.num_layers, batch_first=True, bidirectional=True)
        
        self.fc1_xt = nn.Linear(self.hidden_size*2, self.hidden_size*2)
