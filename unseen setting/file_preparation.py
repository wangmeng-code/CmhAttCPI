...
## load data
def data_formula(root_path, DATASET):
    df = pd.read_csv(root_path + DATASET + '.csv')
    ...
    data.to_csv(root_path + DATASET + '_formulated.csv', index=False)
    return data
...
if __name__ == '__main__':
    ...
    mismatch_score = -1
    root_path = './data/'
    save_path = root_path + 'files_preparation/'
    
    os.makedirs(save_path, exist_ok=True)
    formulated_data = data_formula(root_path, DATASET)
    
    ligand_files_preparation(save_path, formulated_data)
    protein_files_preparation(save_path, formulated_data, gap_open_penalty, gap_extend_penalty, match_score, mismatch_score)
