...
def get_mol_dict(mol_dict_path):
    with open(mol_dict_path + 'mols_dict','rb') as f:
...
def compound_cluster(ligand_ls, mol_ls, thred_ls, save_cluster_path):
    print('compound_cluster...')
    compound_cluster_path = save_cluster_path +  'compound_clusters/'
...
def protein_cluster(protein_ls, idx_ls, thred_ls, prot_sim_mat, save_cluster_path):
    print('protein_cluster...')
    
    protein_cluster_path = save_cluster_path + 'protein_clusters/'
...
if __name__ == "__main__":
    print('*'*20 + ' Step2: clustering for compounds and proteins ' + '*'*20)
    (prepared_files_path, save_cluster_path) = sys.argv[1:]
    
    print('1/3 loading preparation files...')
    mol_dict = get_mol_dict(prepared_files_path)

    print('2/3 generating cpi information...')
    CPI_info_dict = {}
    CPI_data = pd.read_csv(prepared_files_path + 'formulated_data.csv')
    ...
    with open(prepared_files_path + 'all_combined_input', 'wb') as f:
    ...
    compound_cluster(ligand_ls, mol_ls, thred_ls, save_cluster_path)

    # protein_cluster
    prot_sim_mat = np.load(prepared_files_path + 'sim_mat.npy').astype(np.float32)
    ori_protein_list = np.load(prepared_files_path + 'proteins_list.npy', allow_pickle=True).tolist()
    idx_ls = [ori_protein_list.index(pid) for pid in protein_ls]
    protein_cluster(protein_ls, idx_ls, thred_ls, prot_sim_mat, save_cluster_path)
