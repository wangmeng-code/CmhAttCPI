...
def load_data(fold, thred, root_path, dataset, scenario):
    if scenario == 'unseen setting':
        ...
        with open(root_path + 'preprocessing/' +  str(fold) +'fold_' + dataset + '_input','rb') as f:
            data_pack = pickle.load(f)
...
def load_dict(fold, thred, root_path, scenario):
    if scenario == 'unseen setting':
        ...
        f = open(root_path + 'preprocessing/' + str(thred) +'/'+ str(fold) + 'fold_bond_dict','rb')
        bond_dict = pickle.load(f)
        f.close()

        f = open(root_path + 'preprocessing/' + str(thred) +'/'+ str(fold) + 'fold_word_dict','rb')
        word_dict = pickle.load(f)
        f.close()

    else:
        f = open(root_path + 'preprocessing/' + str(fold) + 'fold_atom_dict','rb')
        atom_dict = pickle.load(f)
        f.close()
        
        f = open(root_path + 'preprocessing/' + str(fold) + 'fold_bond_dict','rb')
        bond_dict = pickle.load(f)
        f.close()

        f = open(root_path + 'preprocessing/' + str(fold) + 'fold_word_dict','rb')
        word_dict = pickle.load(f)
        f.close()
