...
if __name__ == '__main__':
    print('*'*20 + ' Step1: preparating files for clustering ' + '*'*30)
    
    (dataset_path, save_prepared_files_path) = sys.argv[1:]
    ...
    os.makedirs(save_prepared_files_path, exist_ok=True)

    df = pd.read_csv(dataset_path, sep=' ',header=None)
    
    formulated_data = data_formula(df)
    formulated_data.to_csv(save_prepared_files_path + 'formulated_data.csv',index=False)
    
    ligand_files_preparation(save_prepared_files_path, formulated_data)
    protein_files_preparation(save_prepared_files_path, formulated_data, gap_open_penalty, gap_extend_penalty, match_score, mismatch_score)
