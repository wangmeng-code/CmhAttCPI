...
class ProcessTestDataSet(Dataset):
    def __init__(self, data_pack):
        self.data_pack = data_pack
    ...
    def __getitem__(self, idx):
        vertex, edge, atom_adj, bond_adj, num_nbs, seq_input, smile, sequence  = [self.data_pack[data_idx][idx] for data_idx in range(8)]    
        
        return [vertex, edge, atom_adj, bond_adj, num_nbs, seq_input, smile, sequence]
        
def Collate_Test_fn(batch_data):
    
    vertex, edge, atom_adj, bond_adj, nbs, seq_input, smile, sequence = [[data[data_idx] for data in batch_data] for data_idx in range(8)]
    vertex_mask = get_mask(vertex)
    ...
    # pad proteins and make masks
    seq_mask = get_mask(seq_input)
    seq_input = pack1D(seq_input)
    ...
    seq_mask = torch.FloatTensor(seq_mask)
    seq_input = torch.LongTensor(seq_input)

    return (vertex_mask, vertex, edge, atom_adj, bond_adj, nbs_mask, seq_mask, seq_input, smile, sequence)
