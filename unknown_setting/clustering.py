...
def compound_cluster(ligand_ls, mol_ls, thred_ls, root_path):
    ...
    compound_cluster_path = root_path +  'cluster_dict/'
...
def protein_cluster(protein_ls, idx_ls, thred_ls, prot_sim_mat, root_path):
    ...
    protein_cluster_path = root_path + 'cluster_dict/'
    ...
    for thred in thred_ls:
        ...
        with open(protein_cluster_path + 'cluster_dict/protein_cluster_dict_' + str(thred),'wb') as f:
...
if __name__ == "__main__":
    ...
    CPI_info_dict = {}
    CPI_data = pd.read_csv(root_path + 'files_preparation/' + DATASET +'_formulated.csv')
