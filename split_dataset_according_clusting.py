...
def split_train_test_clusters(cluster_threshold, cluster_path, Kfolds, SEED):
    with open(cluster_path + 'compound_clusters/compound_cluster_dict_'+str(cluster_threshold), 'rb') as f:
        C_cluster_dict = pickle.load(f)
    
    with open(cluster_path + 'protein_clusters/protein_cluster_dict_'+str(cluster_threshold), 'rb') as f:
...
def load_data(cluster_cross_validation_setting, file_path, cluster_path, cluster_threshold, Kfolds, SEED):
    ...
    if cluster_cross_validation_setting == 'protein_clustering_unknown':
        pair_train_clusters, pair_test_clusters, c_train_clusters, c_test_clusters, p_train_clusters, p_test_clusters, C_cluster_dict, P_cluster_dict = split_train_test_clusters(cluster_threshold, cluster_path, Kfolds, SEED)
    ...
    elif cluster_cross_validation_setting == 'compound_clustering_unknown':
        pair_train_clusters, pair_test_clusters, c_train_clusters, c_test_clusters, p_train_clusters, p_test_clusters, C_cluster_dict, P_cluster_dict = split_train_test_clusters(cluster_threshold, cluster_path, Kfolds, SEED)
        for fold in range(Kfolds):
    ...
    elif cluster_cross_validation_setting == 'both_unknown':
        pair_train_clusters, pair_test_clusters, c_train_clusters, c_test_clusters, p_train_clusters, p_test_clusters, C_cluster_dict, P_cluster_dict = split_train_test_clusters(cluster_threshold, cluster_path, int(Kfolds**0.5), SEED)    
...
if __name__ == '__main__':
    print('*'*20 + ' Step3: spliting dataset into train/val/test set based on clustering ' + '*'*20)
    (prepared_files_path, cluster_path, save_kfold_data_path, setting, cluster_threshold) = sys.argv[1:]
    cluster_cross_validation_setting = setting
    os.makedirs(save_kfold_data_path, exist_ok = True)
    
    Kfolds = 5
    SEED = 1234
    train_rate = 0.89 
    
    data_pack, Train_idx_list, Test_idx_list = load_data(cluster_cross_validation_setting, prepared_files_path, cluster_path, cluster_threshold, Kfolds, SEED)
    ...
    for fold in range(Kfolds):
        ...
        if fold < len(Train_idx_list):
            ...
            train_data[['SMILES','Sequence','Label']].to_csv(save_kfold_data_path + str(fold) + 'fold_' +  str(cluster_threshold) + '_train.txt',sep=' ',index=False,header=None)
            val_data[['SMILES','Sequence','Label']].to_csv(save_kfold_data_path + str(fold) + 'fold_'  + str(cluster_threshold) +'_val.txt',sep=' ',index=False,header=None)
            test_data[['SMILES','Sequence','Label']].to_csv(save_kfold_data_path + str(fold) + 'fold_' + str(cluster_threshold) +'_test.txt',sep=' ',index=False,header=None)
